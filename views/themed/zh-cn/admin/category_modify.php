...
　<form name="myform" id="myform" method="post" action="<?=SITE_URLI?>/admin-category/modify_save" >  
    <input type="hidden" name="cateid" value="<?=$data['id']?>">
	<input type="hidden" name="initparentid" value="<?=$data['parentid']?>">
	<input type="hidden" name="path" value="<?=$data['path']?>">
  <tr> 
    <td class="spec" width="15%" align="right">分类名称:</th> 
    <td><input name="name" type="text" id="name" size="20" maxlength="20" value="<?=$data['name']?>"/></td> 
  </tr>
  <tr> 
    <td class="spec" align="right">所属分类:</th> 
    <td>
	 <select name="parentid" size="20" id="parentid">
	    <option value="0" <?=$data['parentid'] == 0 ? 'selected' : ''?>>一级分类</option>
		<?foreach($tree as $node){?>         
		<option value="<?=$node['id']?>" <?=$data['parentid'] ==$node['id']? 'selected' : ''?>><?=$node['name']?></option>  
        <?}?>
	 </select>
