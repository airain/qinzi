...
/**
 * BasePager
 * base Paginator provide public method.
 *
 * @author hesen2006cn@126.com
 * create_time : 2011-02-15
 *
 * Example usage:
 * <code>
 * Doo::loadHelper('BasePager');
 * Doo::loadModel('User');
 *
 * $page = $this->params['p'];
 * $total = User::getTotal();
 * $pager = new BasePager(Doo::conf()->APP_URL.'main/gallery', $total, 6, 10);
 * $pager->makeUrl(array('p'=>''));
 * $pager->output;
 *
...
.inactivePrev, .inactiveNext{
     ...
     */
    function  __construct($baseURL='', $totalItems=120, $itemPerPage=10, $maxlength=10, $prevText='prev', $nextText='next'){
        ...
        if(!empty($this->baseUrl)){
    			if(strstr($this->baseUrl,'?')=== false && substr($this->baseUrl, -1, 1) != '/')
    ...
    public function paginate($page, $itemPerPage=0){
        ...
        $this->low = (($this->currentPage>0)?($this->currentPage-1):0) * $this->itemPerPage;
     ...
     * Generate a uri.
     *
     * @param array $param Array of uri parameters
     * 
     */
		public function makeUrl($param = null){
     ...
     * 
     */
		public static function getPageNumber(&$param = null,$page_tag='p'){
			
			$current_page = 1;
			if(isset($param[$page_tag]) && !empty($param[$page_tag])){
				$current_page = intval($param[$page_tag]);
			}
			$param[$page_tag] = '';
			return $current_page;
		}
