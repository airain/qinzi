 ...
 * @version 1.0
 */
class SaeTOAuthV2 extends OAuthV2{
	/**
	 * @ignore
	 */
	public $client_id;
	...
	*//**
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		$params['with_offical_account'] = 1;print_r($params);exit;
...
class SaeTClientV2
{
	...
	{
		$this->oauth = new SaeTOAuthV2( $akey, $skey, $access_token, $refresh_token, $debug );
	}
	 ...
	 * 获取用户发布的微博信息列表
	 */
	function user_timeline($page,$limit,$uid_name){
		if(is_int($uid_name)){
			return $this->user_timeline_by_id($uid_name, $page, $limit);
		}else{
			return $this->user_timeline_by_name($uid_name, $page, $limit);
		}
	}
	 ...
	 *  - badge		int	徽章是否可见，0：不可见、1：可见，默认为1。
